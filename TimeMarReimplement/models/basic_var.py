...
__all__ = ['FFN', 'AdaLNSelfAttn', 'AdaLNBeforeHead', 'SelfAttention']
...
class FFN(nn.Module):
    ...
    Args:
        ...
        hidden_features: Hidden layer dimension (default: in_features)
        out_features: Output dimension (default: in_features)
        drop: Dropout probability
        fused_if_available: Use fused MLP operations if available
    """
...
class SelfAttention(nn.Module):
    """Self-attention module with multiple acceleration backends support

    Args:
        block_idx: Identifier for the transformer block
        embed_dim: Embedding dimension
        num_heads: Number of attention heads
        attn_drop: Attention dropout probability
        proj_drop: Projection dropout probability
        attn_l2_norm: Use L2 normalization for attention scores
        flash_if_available: Use FlashAttention if available
    """
    def __init__(
    ...
    def kv_caching(self, enable: bool):
        """Enable/disable key-value caching for autoregressive inference"""
...
class AdaLNSelfAttn(nn.Module):
    """Adaptive Layer Normalization Self-attention Block

    Args:
        block_idx: Transformer block identifier
        last_drop_p: Final dropout probability
        embed_dim: Embedding dimension
        cond_dim: Conditional dimension for adaptive parameters
        shared_aln: Share adaptive parameters across blocks
        norm_layer: Normalization layer type
        num_heads: Number of attention heads
        mlp_ratio: FFN hidden dimension ratio
        drop: General dropout probability
        attn_drop: Attention-specific dropout probability
        drop_path: Stochastic depth probability
        attn_l2_norm: Use L2 norm in attention
        flash_if_available: Use FlashAttention if available
    ...
    def __init__(
        ...
        if self.shared_aln:
            # Shared adaptive parameters across blocks
    ...
    def forward(self, x, cond_BD, attn_bias):   # C: embed_dim, D: cond_dim
        ...
        # print("Caution! Use AdaLNSelfAttn")
        if self.shared_aln:
...
class AdaLNBeforeHead(nn.Module):
    """Final Adaptive Layer Normalization before output head

    Args:
        C: Embedding dimension
        D: Conditional dimension
        norm_layer: Normalization layer type
    """
    ...
    def forward(self, x_BLC: torch.Tensor, cond_BD: torch.Tensor):
        # print("Caution! Use AdaLNBeforeHead")
        # print("x_BLC",x_BLC.shape)
...
class CrossAttention(nn.Module):
    """Cross-attention Module (Potential issues exist)
    ...
    def __init__(self, embed_dim, num_heads, drop=0.1):
        ...
        self.multihead_attn = nn.MultiheadAttention(embed_dim, num_heads, dropout=drop, batch_first = True)
        self.dropout_layer = nn.Dropout(drop)
        self.norm = self.norm = nn.LayerNorm(embed_dim, eps=1e-6)
        self.fc = nn.Linear(embed_dim, embed_dim)  # 
        self.register_buffer('zero_k2_bias', torch.zeros(embed_dim))

    
    def forward(self, x, c_x, attn_bias=None, key_padding_mask=None):
        """Forward pass with potential shape issues"""
        B, L, C = x.shape
        x = self.norm(x)
        # Key/Value from conditional input
        kv = F.linear(c_x, self.mat_kv.weight,
                      bias=torch.cat((self.zero_k2_bias, self.v_bias)))
        kv = kv.view(B, L, 2, self.embed_dim)
        key, value = kv[:, :, 0], kv[:, :, 1]

        # Query from main input
        query = F.linear(x, self.mat_q.weight, self.q_bias)

        # Attention computation with potential mask issues
        attn_output, _ = self.multihead_attn(
            query, key, value,
            attn_mask=attn_bias.squeeze(0).squeeze(0) if attn_bias is not None else None,#  attention bias
