...
def visualization(ori_data, generated_data, analysis,dataset_name, compare=3000):
  ...
  """
    ...
    if analysis == 'tsne':
      ...
      anal_sample_no = ori_data.shape[0]
    ...
    if analysis == 'pca':
        ...
        # plt.title(f'{dataset_name} PCA plot')
        # plt.xlabel('x-pca')
        # plt.ylabel('y_pca')
        plt.legend(loc='upper right',fontsize=14)
        output_dir = "vis_output/pca"
        os.makedirs(output_dir, exist_ok=True)
        plt.rcParams['pdf.fonttype'] = 42
        plt.savefig(f"{output_dir}/{dataset_name}_pca.svg", dpi=100,bbox_inches='tight')
        # plt.ylim((0, 12))
        plt.show()
        plt.close()
        ...
        # ax.legend()
        plt.legend(loc='upper right',fontsize=14)
        # plt.title(f'{dataset_name} t-SNE Visualization')
        # plt.rcParams['pdf.fonttype'] = 42
        output_dir = "vis_output/tsne"
        os.makedirs(output_dir, exist_ok=True)
        plt.rcParams['pdf.fonttype'] = 42
        plt.savefig(f"{output_dir}/{dataset_name}_tsne.svg", dpi=100,bbox_inches='tight')
        # plt.ylim((0, 12))
        plt.show()
        plt.close()
        ...
        plt.legend(loc='upper right',fontsize=14)
        # plt.title(f'{dataset_name} KDE Visualization')
        # plt.xlabel('Data Value')
        # plt.ylabel('Data Density Estimate')
        plt.rcParams['pdf.fonttype'] = 42
        output_dir = "vis_output/kernal"
        os.makedirs(output_dir, exist_ok=True)

        plt.savefig(f"{output_dir}/{dataset_name}_kernel.svg", dpi=100,bbox_inches='tight')
        # plt.ylim((0, 12))
        plt.show()
        # plt.close()
