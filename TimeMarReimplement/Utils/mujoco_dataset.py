...
class MuJoCoDataset(Dataset):
    ...
    def normalize(self, sq):
        ...
        if self.save2npy:
            ...
            file_path = os.path.join(self.dir, f"Mujoco_ground_truth_{self.window}_{self.period}.npy")
            np.save(file_path, sq)

            # if not os.path.exists(file_path):  # 
            #     np.save(file_path, sq)
            # else:
            #     print(f"File already exists: {file_path}. Skipping save operation.")

            if self.auto_norm:
                # np.save(os.path.join(self.dir, f"Mujoco_norm_truth_{self.window}_{self.period}.npy"), unnormalize_to_zero_to_one(data))
                file_path = os.path.join(self.dir, f"Mujoco_norm_truth_{self.window}_{self.period}.npy")
                np.save(file_path, unnormalize_to_zero_to_one(data))
