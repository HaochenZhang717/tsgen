...
class AdaLayerNorm(nn.Module):
    def __init__(self, n_embd):
        super().__init__()
        self.emb = SinusoidalPosEmb(n_embd)  #  n_embd
        self.silu = nn.SiLU()
        self.linear = nn.Linear(n_embd, n_embd * 2)  #  2*n_embd
        self.layernorm = nn.LayerNorm(n_embd, elementwise_affine=False)

    def forward(self, x, timestep, label_emb=None):
        # print(x.size())
        emb = self.emb(timestep)  # : (batch, n_embd)
        if label_emb is not None:
            assert label_emb.shape == emb.shape, "label_emb  (batch, n_embd)"
            emb = emb + label_emb
        emb = self.linear(self.silu(emb))  # : (batch, n_embd*2)
        # print("Linear output shape:", emb.shape)
        # emb = emb.unsqueeze(1)            # : (batch, 1, n_embd*2)
        # print("Unsqueezed emb shape:", emb.shape)
        scale, shift = torch.chunk(emb, 2, dim=2)  #  (batch, 1, n_embd)
